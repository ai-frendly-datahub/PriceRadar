...
def fetch_fallcent_list(url: str) -> list[dict[str, Any]]:
    ...
    for card in soup.select(".product-card"):
        title = (
            (card.select_one(".title") or {}).get_text(strip=True)
            if card.select_one(".title")
            else ""
        )
        price_txt = (
            (card.select_one(".price") or {}).get_text(strip=True)
            if card.select_one(".price")
            else ""
        )
        avg_txt = (
            (card.select_one(".avg-price") or {}).get_text(strip=True)
            if card.select_one(".avg-price")
            else ""
        )
        link_tag = card.select_one("a")
        link = link_tag.get("href") if link_tag else ""
        discount_txt = (
            (card.select_one(".discount") or {}).get_text(strip=True)
            if card.select_one(".discount")
            else ""
        )
